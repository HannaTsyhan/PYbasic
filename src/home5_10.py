...
class DbHelper:
    ...
    def save_to_db(self):
        ...
        cursor.execute('CREATE TABLE IF NOT EXISTS advertising (text text, expiration_date text, days_left);')
        cursor.execute('CREATE TABLE IF NOT EXISTS weather_forecast (text text, region text, is_weather_fine_tomorrow text);')
        for article in self.articles:
            if isinstance(article, News):
                cursor.execute('SELECT * FROM news WHERE text=? AND city=?',  (article.text, article.city))
                if len(cursor.fetchall()) == 0:
                    cursor.execute("INSERT INTO news (text, city, date) VALUES (?, ?, ?)", (article.text, article.city, article.date))
            if isinstance(article, Advertising):
                cursor.execute('SELECT * FROM advertising WHERE text=? AND expiration_date=?', (article.text, article.expiration_date))
                if len(cursor.fetchall()) == 0:
                    cursor.execute(f"INSERT INTO advertising (text, expiration_date, days_left) VALUES (?, ?, ?)", (article.text, article.expiration_date, article.days_left))
            if isinstance(article, WeatherForecast):
                cursor.execute('SELECT * FROM weather_forecast WHERE text=? AND region=?', (article.text, article.region))
                if len(cursor.fetchall()) == 0:
                    cursor.execute(f"INSERT INTO weather_forecast (text, region, is_weather_fine_tomorrow) VALUES (?, ?, ?)", (article.text, article.region, article.is_weather_fine_tomorrow))
...
if __name__ == '__main__':
    ...
    db_helper = DbHelper(articles_objects)
